<template>
    <header class="sticky top-0 z-50 transition-all duration-300">
        <section :class="['w-full',isScrolled ? 'bg-gray-100/50 backdrop-blur shadow-md py-2': 'bg-gray-100 py-4']">
                <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                        <div class="flex flex-col md:flex-row 
                        items-center justify-between gap-3 md:gap-6">
                            <!-- logo + Mobole menu button-->
                            ...
                            <!-- search bar-->
                                <form class="w-full md:flex-1 max-w-sm mx-4" role="search" aria-label="Site Search">
                                    <label class="relative w-full">
                                        <input type="text" placeholder="Search... " 
                                        class="w-full px-4 py-2 text-sm border border-gray-300 rounded-full focus:outline-none 
                                        focus:ring-2 focus:ring-pink-500 transition-all duration-200">
                                        <button type="submit" class="absolute right-2 top-1/2 
                                        -translate-y-1/2 text-gray-500 hover:text-indigo-600">
                                ...
                                <!-- Icons-->
                                 <aside class="flex items-center justify-end space-x-4 w-full md:w-auto">
                                    ...
                                    <button class="relative p-5 text-gray-700 hover:text-pink-600 " aria-label="Account">
        ...
        <nav class="bg-pink-950 " aria-label=" Main navigation">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                ...
                <section v-if="isMobileMenuOpen"
                 class="md:hidden mt-2 bg-white rounded-lg shadow-md
                 p-4 space-y-3 text-[#5D4037] text-center" aria-label="Mobile Navigation">
                <a v-for="item in navItems" :href="item.link" :key="item.id"
                class="block hover:text-amber-600 text-smfont-medium ">{{ item.name }}
            </a>
        </section>
            </div>
